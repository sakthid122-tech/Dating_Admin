          ...
          <div className="reports-list">
            {Object.values(reportsByUser).map((userReports) => {
              ...
              return (
                <ReportCard
                  key={firstReport.id}
                  reportIds={userReports.map(r => r.id)}
                  reportedPhone={firstReport.reported_phone}
                  name={firstReport.reported_name || "Unknown User"}
                  reporter={firstReport.reporter_name || "Unknown Reporter"}
                  reason={firstReport.reason}
                  description={firstReport.message}
...
const ReportCard = ({
  ...
  const handleViewContent = async () => {
    ...
    try {
      // Fetch the full user data from the users table using phone
      const { data, error } = await supabase
        .from("users")
        .select("*")
        .eq("phone", reportedPhone)
        .single();
      ...
      let isSuspended = data.is_suspended || false;
      ...
      // Prepare user object with all necessary fields for UserProfileDetails
      const user = {
        ...data,
        status: isSuspended ? "SUSPENDED" : isActive ? "ACTIVE" : "INACTIVE",
        verified: data.is_verified || false,
        last_active: data.last_seen ? new Date(data.last_seen).toISOString() : "—",
        email: data.phone || data.id, // Use phone as email fallback
      };

      console.log("Navigating to user profile with data:", user);

      // Navigate to UserProfileDetails with user data
    ...
    <div className="report-card">
      ...
      <div className="actions">
        <button 
          ...
          disabled={loading}
        >
          <Eye size={16} /> {loading ? "Loading..." : "View Content"}
        </button>
