...
//             // auto remove suspension if expired
...
//                                                             user.status?.toString().toLowerCase() === "1" ||
//                                                             user.status === true;
...
//                 email: user.phone || user.id, // Use phone as email fallback or id
//                 phone: user.phone,
//                 status: isSuspended ? "SUSPENDED" : isActive ? "ACTIVE" : "INACTIVE",
//                 subscription: user.subscription || "free",
//                 verified: user.is_verified,
//                 last_active: user.last_seen ? new Date(user.last_seen).toISOString() : "—",
//             };
//         });
//     }
...
//         // Use SELECT * to get all columns that exist in your database
//         const { data, error } = await supabase
//             .from("users")
//             .select("*")
//             .order("last_seen", { ascending: false })
...
//             return {
//                 ...user, // Spread all user data from database
//                 status: isSuspended ? "SUSPENDED" : isActive ? "ACTIVE" : "INACTIVE",
//                 verified: user.is_verified || false,
//                 last_active: user.last_seen ? new Date(user.last_seen).toISOString() : "—",
//                 email: user.phone || user.id, // Add email field using phone or id as fallback
//             };
//         });
//     }
...
export default function UsersLegacy() {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter((user) => {
            ...
            const mobile = user.email?.toLowerCase() || ""; // phone stored in email field
                                            ...
                                            filteredUsers.map((user, index) => {
                                                return (
                                                    <tr key={user.id} style={{
                                                        ...
                                                        <td style={tdStyle}>{formatLastActive(user.last_active)}</td>
                                                            ...
                                                            <div style={{
                                                                ...
                                                                <button
                                                                    ...
                                                                    onClick={(e) => {
                                                                        e.preventDefault();
                                                                        e.stopPropagation();
                                                                        navigate("/userProfileDetails", { state: { user } });
                                                                    }}
                                                                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#F3F4F6'}
                                                                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                                                                >
